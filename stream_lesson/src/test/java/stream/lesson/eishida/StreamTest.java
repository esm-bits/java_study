...
import stream.lesson.eishida.StreamTest.Employee.Sex;
...
public class StreamTest {
    ...
    public void 社員を事業部ごとにグループ化し事業部内は年齢で昇順に() throws Exception {
        ...
        Map<Integer, List<Employee>> grouped = employees.stream()
                // あれやこれや
        		.sorted(comparingInt(emp -> emp.getAge()))
        		.collect(groupingBy(emp -> emp.getDepartment()))
        		;
    ...
    public void 男性社員と女性社員の平均年齢を算出() throws Exception {
        ...
        Map<Sex, Double> averages = employees.stream()
                // あれやこれや
        		.collect(groupingBy(emp -> emp.getSex(),
        				averagingDouble(emp -> emp.getAge())))
        		;
