...
import stream.lesson.nhashi.StreamTest.Employee.Sex;
...
public class StreamTest {
  ...
  @Test public void 社員を事業部ごとにグループ化し事業部内は年齢で昇順に() throws Exception {
    ...
    Map<Integer, List<Employee>> grouped = employees.stream()
        .sorted(comparing(Employee::getAge))             // 年齢でソート
        .collect(groupingBy(Employee::getDepartment))    // 事業部でグルーピング
        ;
  ...
  @Test public void 男性社員と女性社員の平均年齢を算出() throws Exception {
    ...
    Map<Sex, Double> averages = employees.stream().collect(groupingBy(Employee::getSex, averagingDouble(Employee::getAge)));
